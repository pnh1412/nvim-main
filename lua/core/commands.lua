...
command("KeymapConflicts", function()
	...
	for key, maps in pairs(seen) do
		if #maps > 1 then
			local mode, lhs = key:match("([^\t]+)\t(.+)")
			table.insert(conflicts, ("%s %s -> %d mappings"):format(mode, lhs, #maps))
		end
	end
	...
	if #conflicts == 0 then
		vim.notify("No active keymap conflicts in the current buffer", vim.log.levels.INFO, { title = "Keymaps" })
		return
	end
	...
	vim.bo[bufnr].filetype = "keymap-conflicts"
	vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, conflicts)
	vim.bo[bufnr].modifiable = false
end, { desc = "Show active keymap conflicts for the current buffer" })
